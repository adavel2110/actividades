...
import { FolderTree, Plus, Pencil, Trash2, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function CategoriesPage() {
  ...
  const goToPage = (p: number) => { setPage(p); loadCategories(p); };
    ...
    <div className="min-h-screen bg-slate-900 flex">
      ...
      <main className="flex-1 p-6 md:p-8 max-w-2xl mx-auto w-full">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2"><FolderTree className="w-5 h-5" /> Categorías</h1>
        ...
        {categories.length === 0 ? (
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-12 text-center text-slate-500">No hay categorías registradas</div>
        ) : (
          <>
            <div className="space-y-2">
              {categories.map((cat) => (
                <div key={cat.id} className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex items-center justify-between">
                  {editing === cat.id ? (
                    <div className="flex gap-2 flex-1 mr-3">
                      <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)}
                        className="flex-1 bg-slate-900 border border-slate-800 rounded-lg py-1.5 px-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                        onKeyDown={(e) => e.key === "Enter" && handleUpdate(cat.id)} />
                      <button onClick={() => handleUpdate(cat.id)} className="text-xs bg-emerald-600 hover:bg-emerald-500 text-white py-1.5 px-3 rounded-lg">Guardar</button>
                      <button onClick={() => setEditing(null)} className="text-xs bg-slate-700 hover:bg-slate-600 text-white py-1.5 px-3 rounded-lg">Cancelar</button>
                    </div>
                  ) : (
                    <>
                      <span className="text-sm font-medium">{cat.name}</span>
                      <div className="flex gap-2">
                        <button onClick={() => { setEditing(cat.id); setEditName(cat.name); }} className="p-1.5 bg-slate-800 hover:bg-slate-700 rounded-lg transition-all"><Pencil className="w-3.5 h-3.5 text-slate-400" /></button>
                        <button onClick={() => handleDelete(cat.id)} className="p-1.5 bg-slate-800 hover:bg-red-500/20 rounded-lg transition-all"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                      </div>
                    </>
                  )}
                </div>
